...
export default function EvaluateBoard (move, prevSum, color) {
...
// --------------------------------------------------
    ...
    if('captured' in move ){
        ...
        if (move.color === color) {
            ...
            positionOfOpponent[move.color][move.captured][to[0]][to[1]];
            prevSum += sumValue;
            
        }else{
        // if the color that was captured does not match Black
        // Our piece was captured by A.I.
            let sumValue = weights[move.captured] +
            positionOfSelf[move.color][move.captured][to[0]][to[1]];
            prevSum -= sumValue;
...
// --------------------------------------------------
    ...
    if (move.flags.includes('p')) {
        ...
        if (move.color === color) {
            ...
            let sumValuePromotion = weights[move.promotion] +
                positionOfSelf[move.color][move.promotion][to[0]][to[1]];
